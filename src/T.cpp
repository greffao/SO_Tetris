...
int T::down(TetrisGrid& gridGame)
{
    int x = centro[X];
    int y = centro[Y];

    int a = gridGame.acessarGrid(x - 1, y + 1, ACCESS);
    ...
    gridGame.acessarGrid(x - 1, y + 1, ID_T);
    gridGame.acessarGrid(x + 1, y + 1, ID_T);
    gridGame.acessarGrid(x, y + 2, ID_T);
...
void T::left(TetrisGrid& gridGame)
{
    int x = centro[X];
    int y = centro[Y];
    ...
    {
        gridGame.acessarGrid(x - 2, y, ID_T);
        gridGame.acessarGrid(x - 1, y + 1, ID_T);
...
void T::right(TetrisGrid& gridGame)
{
    int x = centro[X];
    int y = centro[Y];
